...
void MainWindow::setSocket(ClientSocket *tcpSocket, const QString &name)
{
    if(tcpSocket!=NULL)
    {
        ...
        QJsonArray array=DataBaseMagr::Instance()->GetMyFriend(MyApp::m_nId);
        m_tcpSocket->SltSendMessage(GetMyFriends,array);
...
void MainWindow::PraseAddFriendRequist(const QJsonValue &dataVal)
{
    if(dataVal.isObject())
    {
        ...
        if(!DataBaseMagr::Instance()->isMyFriend(MyApp::m_nId,name))
        {
            ...
            qDebug()<<"MyApp::m_nid"<<MyApp::m_nId<<"好友ID"<<id;
...
void MainWindow::SltFriendChatWindowClose()
{
    ...
    disconnect(window,&ChatWindow::signalSendMessage,m_tcpSocket,&ClientSocket::SltSendMessage);
    disconnect(window,&ChatWindow::signalClose,this,&MainWindow::SltFriendChatWindowClose);
    ...
    m_chatFriendWindows.removeOne(window);
...
void MainWindow::ParseGroupMessageReply(const QJsonValue &dataVal)
{
    if(dataVal.isObject())
    {
        QJsonObject jsonObject=dataVal.toObject();
        int nId=jsonObject.value("group").toInt();
...
void MainWindow::ParseGetFriendsReply(const QJsonValue &dataVal)
{
    ...
    if(dataVal.isArray())
    {
        ...
        for(int i=0;i<size;i++)
        {
            ...
            {
                QJsonObject obj;
                obj.insert("from",MyApp::m_nId);
                obj.insert("id",-2);
                obj.insert("msg",strHead);
                m_tcpSocket->SltSendMessage(GetFile,obj);
                myHelper::Sleep(100);
            }
            QQCell *cell=new QQCell;
            cell->groupName=QString(tr("我的好友"));
...
void MainWindow::SltReadMessages(const QJsonValue &json, const int &id)
{
    ...
    // 没有检索到聊天窗口，直接弹出新窗口
    QList<QQCell*> group=ui->frindListWidget->getCells();
    for(QQCell *cell:group.at(0)->childs)
    {
        if(cell->id=id)
        {
            ChatWindow *window=new ChatWindow();
            connect(window, SIGNAL(signalSendMessage(quint8,QJsonValue)), m_tcpSocket, SLOT(SltSendMessage(quint8,QJsonValue)));
...
void MainWindow::ParseAddFriendRequest(const QJsonValue &dataVal)
{
    if (!dataVal.isObject())
    {
        ...
        if(!DataBaseMagr::Instance()->isMyFriend(MyApp::m_nId,name))
        {
            ...
            cell->status    = OnLine;
            ui->frindListWidget->insertQQCell(cell);
            DataBaseMagr::Instance()->AddFriend(nId,MyApp::m_nId,name);
        }
        // 播放系统提示音
...
{
    switch (type) {
    case AddFriend:  //服务器返回，主动添加好友的用户消息
        PraseAddFriendReply(dataVal);
        break;
    case AddFriendRequist:  //服务器返回，主动添加好友的用户消息
        PraseAddFriendRequist(dataVal);
        break;
    case UserOnLine:
    {
